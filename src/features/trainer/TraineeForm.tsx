...
export function TraineeForm() {
  ...
  const liveNutrition = useMemo(() => {
    ...
    const macros = calculateMacros(formData.weight_kg, Math.max(0, goalCalories), formData.protein_factor || 2.0, formData.fat_percentage || 25);
    ...
    <div className="bg-slate-800 text-white p-6 rounded-2xl shadow-xl">
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="flex justify-between items-center bg-slate-900/50 p-2.5 rounded-lg border border-slate-700/50">
            <span className="text-slate-300">שומן <span className="text-xs text-slate-500">({formData.fat_percentage || 25}%)</span></span>
          ...
          <div className="md:col-span-7 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-5">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div className="col-span-2 sm:col-span-3 mt-4 sm:mt-0">
                  <label className="block text-sm font-bold text-slate-700 mb-1">יעד חלבון (גרם לק״ג משקל גוף)</label>
                  <div className="flex bg-slate-100 p-1 rounded-lg">
                    {[1.8, 2.0, 2.2].map(factor => (
                      <button
                        key={factor}
                        type="button"
                        onClick={() => setFormData({ ...formData, protein_factor: factor })}
                        className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
                          (formData.protein_factor || 2.0) === factor
                ...
                <div className="col-span-2 sm:col-span-3 mt-4 sm:mt-0">
                  <label className="block text-sm font-bold text-slate-700 mb-1">אחוז שומן (מתוך שאר הקלוריות)</label>
                  <div className="flex bg-slate-100 p-1 rounded-lg">
                    {[20, 25, 30].map(pct => (
                      <button
                        key={pct}
                        type="button"
                        onClick={() => setFormData({ ...formData, fat_percentage: pct })}
                        className={`flex-1 py-2 text-sm font-medium rounded-md transition-all ${
                          (formData.fat_percentage || 25) === pct
            ...
            <label dir="ltr" className="flex w-full h-auto items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 cursor-pointer transition-colors hover:bg-amber-100">
              <input
                type="checkbox"
                name="is_busy_lifestyle"
                checked={formData.is_busy_lifestyle || false}
                onChange={handleCheckboxChange}
                className="h-5 w-5 flex-shrink-0 rounded accent-amber-500 cursor-pointer"
              />
