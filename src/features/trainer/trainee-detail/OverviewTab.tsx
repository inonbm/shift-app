      ...
      isEditing ? (
        <div className="space-y-4">
          <div className="flex flex-col gap-4">
            ...
            <div className="mt-2">
              <label className="block text-xs font-semibold text-slate-500 mb-1">אחוז שומן (משאר הקלוריות)</label>
              <div className="flex bg-slate-100 p-1 rounded-lg">
                {[20, 25, 30].map(pct => (
                  <button
                    key={pct}
                    type="button"
                    onClick={() => setEditForm(prev => ({ ...prev, fat_percentage: pct }))}
                    className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${
                      (editForm.fat_percentage || 25) === pct
