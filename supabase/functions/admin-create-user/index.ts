  ...
  try {
    ...
    if (!email || !password || !full_name || !role) {
      return new Response(JSON.stringify({
        error: 'Missing required parameters (email, password, full_name, role)'
      }), {
        status: 200,
        headers: {
          ...corsHeaders,
    ...
    const { data: currentProfile, error: profileError } = await callerClient.from('profiles').select('role').eq('id', currentUser.id).single();
    if (profileError || currentProfile?.role !== 'admin') {
      return new Response(JSON.stringify({
        error: 'Unauthorized. Only admins can create users via this endpoint.'
      }), {
        status: 200,
        headers: {
          ...corsHeaders,
